...
/**
 * Get full DOM path with classes and IDs
 */
...
export function getComprehensiveComputedStyles(element: HTMLElement): Record<string, string> {
  ...
  // Comprehensive list of important CSS properties
  const properties = [
    'color',
    'background-color',
    'border-color',
...
/**
 * Calculate position details including percentage from left
 */
export function getPositionDetails(
  ...
  return {
    ...
    width: rect.width,
    height: rect.height,
    percentageFromLeft: Math.round(percentageFromLeft * 10) / 10, // Round to 1 decimal
    pixelsFromTop: Math.round(absoluteY),
  };
}

/**
 * Get element description for multi-element selections
 * This would be used if we support selecting multiple elements at once
 */
...
/**
 * Get text content safely (handles various element types)
 */
export function getElementContext(element: HTMLElement): string {
  // For text nodes or elements with direct text
